		...
		<h2>MANAGE DATABASE</h2>
		 ...
		 <div>				
			<form class="user" method=post enctype="multipart/form-data" >
				<div>
				<span>
					<input class='btn btn-primary btn-sm' type=submit name='backup' value="Backup Database" onclick="return confirm('Execute backup now?')" /> &nbsp;
					
					<?php if($_SESSION['ACCOUNT_TYPE']=="Administrator"){
						echo"
						<input class='btn btn-danger btn-sm' type=submit name='erase' value='Wipe All Data' onclick=\"return confirm('Are you sure you want to erase all Data?')\" />
						";
						}
					?>
					&nbsp;
					<input class='btn btn-success btn-sm' type=button value="Restore Database" onclick="$('#file').click();" />
					<input type=file name='file' id='file' onchange="$('#upload').click();" style="visibility:hidden" />
					<input type=submit value='Submit' name='upload' id="upload" style="visibility:hidden" />
				</span>
				</div>
			</form>
		</div><br>
...
if(isset($_POST["backup"])){
	...
	class Backup_Database {
		...
		public function __construct($dbHost, $dbUser, $dbPass, $dbName, $charset = 'utf8') {
			...
			$this->backupFile              = 'WPH-'.$this->dbName.'-'.date("Ymd_His", time()).'.sql';
	...
	if (php_sapi_name() != "cli") {
		echo '<div style="margin:-50px 20px 20px 20px;font-family: monospace;background:#000;color:#FFF;border-radius:5px;padding:20px">';
		echo '<a href="backup.php"><input class="btn btn-danger btn" type=button value=" Close Console " /></a><br><br>';
	}
...
//RESTORE
	...
	//	fclose($fh); 
		...
		set_time_limit(900); // 15 minutes
		if (php_sapi_name() != "cli") {
		echo '<div style="text-align:left;margin:-50px 20px 20px 20px;font-family: monospace;background:#000;color:#FFF;border-radius:5px;padding:20px">';
