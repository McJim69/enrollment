<?php
	...
	$sID=$_GET['id'];
	$exs = $conn->query("SELECT * FROM tblstudent s WHERE s.S_ID='".$sID."'");
?>
					...
					<div id='buttons1' class='col col-md-5 no-print' style='margin-top:15px;'>
						<div class='action-btn-grid' style='display:flex;flex-direction:column;gap:10px;'>
							...
							<div style='display:flex;gap:10px;'>
								...
								<button class='btn btn-default' onclick=\"jump('editStudent.php?id=$rss[0]')\" style='flex:1;'><i class='fas fa-edit'></i> Edit</button>
							</div>
						</div>
					</div>
					<div class='col col-md-7 user-info-box' style='border:1px solid var(--border-color);border-radius:8px;padding:12px;margin-top:15px;'>
						<div><b>".PTITLE."</b></div>
